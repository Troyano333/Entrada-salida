...
export default function App() {
  ...
  const scanAnimationRef = useRef<number | null>(null); // Cambiado de scanInterval a requestAnimationFrame
  ...
  // --- 1. GESTIÓN DE CÁMARA (OPTIMIZADO PARA VELOCIDAD) ---
    ...
    try {
      ...
      const stream = await navigator.mediaDevices.getUserMedia({
        ...
        video: {
          ...
          width: { ideal: 640 }, // Reducido de 1280 a 640 para procesamiento más rápido

          height: { ideal: 480 }, // Reducido de 720 a 480 para procesamiento más rápido

          frameRate: { ideal: 30 } // Agregado para limitar FPS y reducir carga
  ...
  const startNativeScan = () => {
    ...
    const scanLoop = async () => {
      ...
      if (showCamera && !isScanning && video.readyState === video.HAVE_ENOUGH_DATA) {
        ...
        try {
          ...
          if (canvas) {
            ...
            const context = canvas.getContext('2d', { willReadFrequently: true }); // Optimización para getImageData frecuente
            ...
            if (context) {
              ...
              if (barcodes.length > 0) {
                ...
                return; // Salir del loop si se detecta
      ...
      scanAnimationRef.current = requestAnimationFrame(scanLoop); // Usa RAF para mejor rendimiento en lugar de setInterval
  ...
  const startZXingScan = (stream: MediaStream) => {
    ...
    codeReader.current.decodeFromStream(
      ...
      (result: any, err: any) => {
        ...
        if (result) {
          ...
          processScanCode(result.getText());

        }

        if (err && err.name !== 'NotFoundException') { // Ignora errores comunes de "no code found" para no spamear consola
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (nativeMode) {
        ...
        const result = await codeReader.current.decodeFromImageFile(file);
    ...
    <div className={`min-h-screen ${getBgColor()} transition-colors duration-500 text-white font-sans overflow-hidden flex flex-col items-center justify-center p-4`}>
      ...
      {showCamera && (
        ...
        <div className="fixed inset-0 bg-black/95 z-50 flex flex-col items-center justify-center">
          ...
          <div className="relative w-full max-w-md aspect-square bg-black border-2 border-blue-500 rounded-lg overflow-hidden mb-6 shadow-2xl shadow-blue-900/50">
          ...
          <div className="bg-slate-800 px-6 py-2 rounded-full flex items-center gap-2">

            {nativeMode ? <><Zap className="w-4 h-4 text-green-400" /><span className="text-green-400 font-bold text-sm">Modo Nativo (QR y Barras)</span></> : <span className="text-yellow-400 font-bold text-sm">Modo Web (QR y Barras)</span>}
      ...
      <div className="absolute top-4 left-4 flex items-center gap-4 opacity-80">
        ...
        <div className="flex items-center gap-2"><Scan className="w-6 h-6" /><span className="font-mono text-sm">GATEKEEPER v16.7</span></div>
      ...
      {appState === STATE.WAITING && (
        ...
        <div className="text-center w-full max-w-md">
          ...
          <div className="flex flex-col gap-4 w-full max-w-xs mx-auto">
            ...
            <button onClick={() => setShowCamera(true)} className="bg-blue-600 hover:bg-blue-500 text-white px-6 py-4 rounded-xl font-bold flex items-center justify-center gap-3 shadow-lg active:scale-95 transition-all"><Camera className="w-6 h-6" /> ACTIVAR CÁMARA</button>
      ...
      {appState === STATE.DASHBOARD && (
        ...
        <div className="w-full max-w-3xl bg-slate-900 border border-slate-700 rounded-2xl p-6 shadow-2xl h-[80vh] flex flex-col">
          ...
          <div className="flex justify-between items-center mb-6 border-b border-slate-700 pb-4">
            ...
            <h2 className="text-xl font-bold flex items-center gap-2 text-slate-200"><Grid className="w-6 h-6 text-blue-500" /> PANEL DE VIGILANTES</h2>

            <button onClick={() => {setAppState(STATE.WAITING); setDashboardSearch(''); setCurrentUser(null);}} className="bg-slate-800 hover:bg-slate-700 text-white px-4 py-2 rounded-lg text-sm"><X className="w-4 h-4 inline mr-1" /> Cerrar</button>
          ...
          <div className="flex gap-2 mb-6">
            ...
            <div className="relative flex-1">
              ...
              <input type="number" autoFocus className="w-full bg-slate-800 border border-slate-600 rounded-xl py-3 px-4 text-white placeholder-slate-500 focus:border-blue-500 outline-none" placeholder="Buscar cédula..." value={dashboardSearch} onChange={(e) => setDashboardSearch(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && fetchUser(dashboardSearch, true)} />

            </div>

            <button onClick={() => fetchUser(dashboardSearch, true)} className="bg-blue-600 hover:bg-blue-500 text-white px-6 rounded-xl font-bold">BUSCAR</button>
            ...
            {currentUser ? (
              ...
              <div className="animate-fadeIn">
                ...
                <button onClick={() => { setHasLaptop(true); setNewUserForm({ name: String(currentUser.nombre), assetDesc: '', assetId: '' }); setAppState(STATE.NEW_USER_MODE); }} className="w-full mt-6 py-3 border border-dashed border-slate-600 rounded-xl text-slate-400 hover:text-white hover:border-slate-400 hover:bg-slate-800 flex items-center justify-center gap-2">
                  ...
                  <Plus className="w-4 h-4" /> Registrar Nuevo Equipo
              ...
              <div className="h-full flex flex-col items-center justify-center text-slate-600">
                ...
                <p>Ingrese cédula para gestionar.</p>
        ...
        <div className="w-full max-w-2xl bg-slate-800 rounded-2xl p-8 border border-slate-600 shadow-2xl">
          ...
          <div className="flex justify-between items-start border-b border-slate-700 pb-6 mb-6">
            ...
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <h2 className="text-3xl font-bold text-white">{String(currentUser.nombre)}</h2>

                <p className="text-slate-400 font-mono text-lg">ID: {String(currentUser.cedula)}</p>

                <span className={`inline-block px-3 py-1 rounded-full text-sm font-bold mt-2 ${currentUser.estado === 'ACTIVO' ? 'bg-emerald-900 text-emerald-300' : 'bg-red-900 text-red-300'}`}>{String(currentUser.estado)}</span>
          ...
          <div className="space-y-6 text-center">
            ...
            <div className="bg-slate-700/50 p-6 rounded-xl border border-slate-600">
              ...
              <Laptop className="w-10 h-10 mx-auto text-cyan-400 mb-2" />

              <p className="text-cyan-300 font-bold mb-4">SÍ TRAE PORTÁTIL</p>

              <button onClick={() => setShowCamera(true)} className="bg-slate-800 hover:bg-slate-900 text-white px-6 py-3 rounded-lg font-bold flex items-center justify-center gap-2 mx-auto border border-slate-500">

                <Camera className="w-5 h-5" /> ESCANEAR QR/BARRAS

              </button>

            </div>

            <button onClick={handleManualEntry} className="w-full py-4 bg-slate-700 hover:bg-slate-600 rounded-xl font-bold text-slate-200 flex items-center justify-center gap-2 border border-slate-500">

              <UserCheck className="w-5 h-5" /> ENTRADA PEATONAL (SIN EQUIPO)
      ...
      {appState === STATE.NEW_USER_MODE && (
        ...
        <div className="w-full max-w-lg bg-slate-800 rounded-2xl p-8 border-2 border-yellow-500 shadow-2xl">
          ...
          <div className="text-center mb-6">
            ...
            <User className="w-16 h-16 mx-auto text-yellow-500 mb-2" />

            <h2 className="text-2xl font-bold text-white">REGISTRO INICIAL</h2>

            <div className="flex items-center justify-center gap-2 mt-4 bg-slate-900/50 p-2 rounded-lg border border-slate-700">

              <span className="text-slate-400 text-sm font-bold">CÉDULA:</span>

              <input type="number" className="bg-transparent text-yellow-400 font-mono text-lg font-bold text-center outline-none w-40 border-b border-slate-600 focus:border-yellow-500" value={currentUser?.cedula || ''} onChange={(e) => setCurrentUser({...currentUser, cedula: e.target.value})} />
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input type="text" autoFocus className="w-full bg-slate-900 border border-slate-600 rounded p-3 text-white focus:border-yellow-500 outline-none" placeholder="Escriba nombre..." value={newUserForm.name} onChange={e => setNewUserForm({...newUserForm, name: e.target.value})} />

            </div>

            <div className="flex items-center gap-3 p-3 bg-slate-700/30 rounded-lg border border-slate-600 cursor-pointer" onClick={() => setHasLaptop(!hasLaptop)}>

              <div className={`w-6 h-6 rounded border flex items-center justify-center ${hasLaptop ? 'bg-yellow-500 border-yellow-500' : 'border-slate-400'}`}>

                {hasLaptop && <CheckCircle className="w-4 h-4 text-black" />}

              </div>

              <span className="text-sm font-bold text-white">¿Registrar Portátil?</span>
            ...
            {hasLaptop ? (
              ...
              <>
                ...
                <div className="p-4 bg-slate-700/50 rounded-lg border border-slate-600">
                  ...
                  <label className="block text-sm text-yellow-400 font-bold mb-2 uppercase flex items-center gap-2">

                    <QrCode className="w-4 h-4" /> SERIAL / ID DEL EQUIPO

                  </label>

                  <input type="text" className="w-full bg-slate-900 border border-slate-600 rounded p-3 text-white font-mono text-center focus:border-yellow-500 outline-none" placeholder="Ej: SN-554422" value={newUserForm.assetId} onChange={e => setNewUserForm({...newUserForm, assetId: e.target.value})} />

                </div>

                <div>

                  <label className="block text-sm text-slate-400 mb-1">Descripción (Marca/Color)</label>

                  <input type="text" className="w-full bg-slate-900 border border-slate-600 rounded p-3 text-white" placeholder="Ej: HP Pavilion Azul" value={newUserForm.assetDesc} onChange={e => setNewUserForm({...newUserForm, assetDesc: e.target.value})} />
            ...
            <button onClick={registerNewUserAndAsset} className="w-full bg-yellow-600 hover:bg-yellow-500 text-white font-bold py-4 rounded-lg mt-4 flex items-center justify-center gap-2">

              <Plus className="w-5 h-5" /> GUARDAR Y GENERAR
      ...
      {appState === STATE.SHOW_CODE && generatedCodeData && (
        ...
        <div className="w-full max-w-md bg-white rounded-3xl p-8 shadow-2xl text-center text-slate-800 animate-bounce-slow printable-area">
          ...
          <div className="bg-white p-4 border-4 border-slate-900 rounded-xl inline-block mb-4 shadow-lg min-w-[250px]">
            ...
            <img src={screenQrUrl} alt="QR Maestro" className="w-56 h-56" />
          ...
          <div className="grid grid-cols-2 gap-3 no-print">
            ...
            <button onClick={handleDownloadCode} className="w-full bg-blue-600 text-white font-bold py-3 rounded-xl hover:bg-blue-700 flex items-center justify-center gap-2">

              <Download className="w-5 h-5" /> DESCARGAR

            </button>

            <button onClick={handlePrint} className="w-full bg-slate-700 text-white font-bold py-3 rounded-xl hover:bg-slate-800 flex items-center justify-center gap-2">
          ...
          <button onClick={() => showFeedback('success', 'Proceso finalizado.')} className="w-full mt-4 text-slate-400 font-bold py-3 hover:text-slate-600 flex items-center justify-center gap-2 no-print">

            <ArrowRight className="w-5 h-5" /> CERRAR
